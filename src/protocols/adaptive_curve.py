...
from src.protocols.solidity_maths import MathLib
...
class AdaptiveCurveIrm:
    ...
    def _curve(self, rate_at_target: int, err: int) -> int:
        ...
        return MathLib.w_mul_to_zero(
            MathLib.w_mul_to_zero(coeff, err) + self.WAD, rate_at_target
        )

    def _new_rate_at_target(
        self, start_rate_at_target: int, linear_adaptation: int
    ) -> int:
    ...
    def plot_rates(self):
        ...
        rates = [
            r * self.SECONDS_PER_YEAR * 100 / self.WAD for _, r in self.memoized_rates
        ]
...
if __name__ == "__main__":
    ...
    for _ in range(50):
        ...
        target_debt = total_supply * (irm.TARGET_UTILIZATION - irm.WAD // 10) // irm.WAD
